...
namespace Telephone
{
    ...
    {
        SqlConnection con = new SqlConnection(@"Data Source=(LocalDB)\MSSQLLocalDB;AttachDbFilename=C:\Users\David\Documents\Phone.mdf;Integrated Security=True;Connect Timeout=30");
        public Phone()
        {
            InitializeComponent();
        }
        ...
        {
            SqlDataAdapter sda = new SqlDataAdapter("Select * from [Table] Where (Mobile like '%" + textBox5.Text + "%') or (First like '%" + textBox5.Text + "%')  or (Last like '%" + textBox5.Text + "%') ", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            dataGridView1.Rows.Clear();
            foreach (DataRow item in dt.Rows)
